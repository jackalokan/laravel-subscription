# Abovesky Subscription

**Abovesky Subscription** is a flexible plans and subscription management system for Laravel.
...
1. Install the package via composer:
    ```shell
    composer require abovesky/laravel-subscription
    ```
...
**Abovesky Subscription** has been specially made for Eloquent and simplicity has been taken very serious as in any other Laravel related aspect. To add Subscription functionality to your User model just use the `\Abovesky\Subscription\Traits\HasSubscriptions` trait like this:

```php
namespace App\Models;

use Abovesky\Subscription\Traits\HasSubscriptions;
...
```php
$plan = app('abovesky.subscription.plan')->create([
    'name' => 'Pro',
...
```php
$plan = app('abovesky.subscription.plan')->find(1);
...
$amountOfPictures = app('abovesky.subscription.plan_feature')->where('slug', 'pictures_per_listing')->first()->value;

// Get feature value through the subscription instance
$amountOfPictures = app('abovesky.subscription.plan_subscription')->find(1)->getFeatureValue('pictures_per_listing');
```
...
```php
$plan = app('abovesky.subscription.plan')->find(2);
$subscription = app('abovesky.subscription.plan_subscription')->find(1);
...
```php
// Find plan feature
$feature = app('abovesky.subscription.plan_feature')->where('name', 'listing_duration_days')->first();
...
$subscriptions = app('abovesky.subscription.plan_subscription')->byPlanId($plan_id)->get();

// Get bookings of the given user
$user = \App\Models\User::find(1);
$bookingsOfUser = app('abovesky.subscription.plan_subscription')->ofUser($user)->get(); 

// Get subscriptions with trial ending in 3 days
$subscriptions = app('abovesky.subscription.plan_subscription')->findEndingTrial(3)->get();

// Get subscriptions with ended trial
$subscriptions = app('abovesky.subscription.plan_subscription')->findEndedTrial()->get();

// Get subscriptions with period ending in 3 days
$subscriptions = app('abovesky.subscription.plan_subscription')->findEndingPeriod(3)->get();

// Get subscriptions with ended period
$subscriptions = app('abovesky.subscription.plan_subscription')->findEndedPeriod()->get();
```

### Models

**Abovesky Subscription** uses 4 models:
...
(c) 2021 Abovesky, Some rights reserved.
